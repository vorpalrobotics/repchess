...
// --- Phase AU: chess.com importer keeps full metadata (normalized to the same
//     shape Lichess games use), and a re-import clears ONLY chess.com games,
//     leaving Lichess untouched. ---
const appAU = await launchApp();
try {
  // 148. normalizeChessComGame maps an archive object to the Lichess-shaped
  ...
  try {
    const norm = await appAU.page.evaluate(() => {
      const g = {
  ...
  // 149. A draw maps to winner undefined / status draw.
  try {
    const norm = await appAU.page.evaluate(() => window.__importTestHooks.normalizeChessComGame({
  ...
  try {
    ...
    const before = await appAU.page.evaluate(() => window.__importTestHooks.getGames('u1'));
    assert(before.length === 3, `test setup issue: expected 3 games stored, got ${before.length}`);
    const removed = await appAU.page.evaluate(() => window.__importTestHooks.clearChessComGames('u1'));
    assert(removed === 2, `expected 2 chess.com games removed (legacy + enriched), got ${removed}`);
    const after = await appAU.page.evaluate(() => window.__importTestHooks.getGames('u1'));
