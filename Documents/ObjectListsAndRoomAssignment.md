...
- VR: for each slot on an assigned wall, if `assetId` null render a **word plaque**
  ...
  room's own walk order (left → right); the center/anchor slot is excluded
  from list-driving — its pair is the arrival move (the same pair the
  previous room's own door object already shows), not a step of walking
  this room's own sequence — EXCEPT `room.entryNoStreet` (the castle's own
  entry room, walked with no street building to show its entry pair on
  instead), which has nowhere else that pair is shown at all, so it's
  list-drivable in-room there, same exception `computeFullyDecorated` and
  `buildSlots`' render-skip already carve out for it.
